...
const Home = () => {
  ...
  useEffect(() => {
    // Simulate network fetch for skeleton loaders demo
    const timer = setTimeout(() => setIsLoadingRestaurants(false), 1200);
        ...
        <div className="max-w-7xl mx-auto text-center relative z-10" ref={heroRef}>
          ...
          >
            ...
            {showSearchDropdown && (heroSearch || recentSearches.length > 0) && (
              <div id="search-dropdown" role="listbox" className="absolute top-full left-0 w-full mt-4 bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden text-left z-40">
                  ...
                  <div className="p-4">
                    ...
                    {recentSearches.map((recent, idx) => (
                      <div 
                        ...
                        role="option"
                        tabIndex={0}
                        onKeyDown={(e) => { if (e.key === 'Enter') { setHeroSearch(recent); handleExecuteSearch(recent); } }}
                        onClick={() => { setHeroSearch(recent); handleExecuteSearch(recent); }}
                        className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 focus:bg-gray-50 focus:outline-none rounded-xl cursor-pointer transition text-gray-700"
                      >
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center">
              ...
              <div className="w-full md:w-1/3">
                <div className="aspect-video md:aspect-square rounded-2xl overflow-hidden shadow-xl border-4 border-white">
                  <img loading="lazy" src={recommendedRestaurant.image} alt={recommendedRestaurant.name} className="w-full h-full object-cover" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div ref={restaurantsRef}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-end sm:items-center">
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium transition ${deliveryMode === 'delivery' ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900 shadow-sm' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'}`}
                >
                  Delivery
                </button>
                <button 
                  onClick={() => setDeliveryMode('pickup')}
                  className={`px-4 py-2 rounded-full text-sm font-medium transition ${deliveryMode === 'pickup' ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900 shadow-sm' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'}`}
                >
                  Pickup
                </button>
              </div>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <button 
              ...
              className={`px-4 py-2 rounded-full text-sm font-bold transition ${dietFilter === 'All' ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'}`}
            >
              All
            </button>
            <button 
              onClick={() => setDietFilter('Vegan')}
              className={`px-4 py-2 rounded-full text-sm font-bold transition flex items-center gap-2 ${dietFilter === 'Vegan' ? 'bg-green-600 text-white border-green-600' : 'bg-white dark:bg-slate-800 text-green-700 dark:text-green-400 border border-green-200 dark:border-slate-700 hover:bg-green-50 dark:hover:bg-slate-700'}`}
            >
              <Leaf size={16} /> Vegan
            </button>
            <button 
              onClick={() => setDietFilter('Vegetarian')}
              className={`px-4 py-2 rounded-full text-sm font-bold transition flex items-center gap-2 ${dietFilter === 'Vegetarian' ? 'bg-green-500 text-white border-green-500' : 'bg-white dark:bg-slate-800 text-green-600 dark:text-green-400 border border-green-200 dark:border-slate-700 hover:bg-green-50 dark:hover:bg-slate-700'}`}
            >
              <Leaf size={16} /> Vegetarian
            </button>
            <button 
              onClick={() => setDietFilter('Gluten-Free')}
              className={`px-4 py-2 rounded-full text-sm font-bold transition flex items-center gap-2 ${dietFilter === 'Gluten-Free' ? 'bg-amber-500 text-white border-amber-500' : 'bg-white dark:bg-slate-800 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-slate-700 hover:bg-amber-50 dark:hover:bg-slate-700'}`}
            >
          ...
          {isLoadingRestaurants ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3, 4, 5, 6].map((n) => (
                <div key={n} className="bg-white dark:bg-slate-800 rounded-[2rem] overflow-hidden card-shadow border border-slate-100 dark:border-slate-700 h-[380px] animate-pulse">
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex justify-between border-t border-slate-100 dark:border-slate-700 pt-5">
                      <div className="h-8 bg-slate-200 dark:bg-slate-700 rounded-lg w-24"></div>
                      <div className="h-5 bg-slate-200 dark:bg-slate-700 rounded w-16 mt-1.5"></div>
                    </div>
                  </div>
                </div>
              ))}
